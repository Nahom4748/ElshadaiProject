...
const YearlyDropdown = () => {
  ...
  return (
    <div className="p-6 bg-gradient-to-b from-gray-50 to-white rounded-lg shadow-lg lg:col-span-9">
      {/* Header */}
      ...
      {Object.entries(yearsData).map(([yearKey, year]) => (
        <details
          key={yearKey}
          className={`mb-6 rounded-lg overflow-hidden shadow-md border ${
          ...
          <ul className="mt-2 space-y-2 py-4 px-6 bg-white">
            {year.quarters.map((quarter, idx) => (
              <li key={idx}>
                ...
                >
                  <span className="flex items-center space-x-3">
                    <FaArrowRight className="text-lg text-blue-600 transition-transform duration-300 transform group-hover:translate-x-2 group-hover:text-blue-800" />
