...
function FirstYearQuarterThree() {
  ...
  const [hasPaidQ1, setHasPaidQ3] = useState(false);
  const { user } = useAuth();

  useEffect(() => {
    // Fetch lessons for Quarter 1
    axios
      .get("http://localhost:5001/api/quarter3/videos")
      .then((response) => {
    ...
    axios
      ...
      .then((response) => {
        const hasPaid = response.data.some(
          (payment) => payment.quarter === "Q3" && payment.status === 1
        );
        setHasPaidQ3(hasPaid);
      })
        ...
        <div className="flex items-center justify-center h-screen">
          <div className="text-center p-8 bg-gradient-to-r from-red-800 to-red-600 shadow-lg rounded-lg border border-red-700">
            ...
            <p className="text-gray-200 text-lg">
              Complete the payment for Quarter 3 to unlock this content.
            </p>
          </div>
        </div>
      )}
...
export default FirstYearQuarterThree;
